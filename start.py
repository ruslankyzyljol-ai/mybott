...
def register_start(bot):
    ...
    def start(msg):
        ...
        if len(args) > 1 and args[1].startswith("REF"):
            ...
            if ref_id != str(user_id) and not user.get("referred_by"):
                ...
                if days > 0:
                    try:
                        bot.send_message(int(ref_id), f"🎉 Жаңы дос кирди! {days} күн VIP алдыңыз!")
                    except:
                        pass

        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton("📢 Каналга жазыл", url=f"https://t.me/{CHANNEL_ID.replace('@','')}"))
    ...
    def check_sub(call):
        ...
        if check_subscribed(user_id):
            ...
            bot.send_message(user_id, "🎉 Кош келдиңиз! Меню төмөндө 👇", reply_markup=main_menu())
        else:
    ...
    def profile(msg):
        user_id = msg.from_user.id
        user = get_user(user_id)
        vip = is_vip_check(user_id)
        refs = len(user.get("referrals", []))
        vip_text = "👑 VIP: ✅" if vip else "👑 VIP: ❌"
        bot.send_message(user_id,
            f"👤 *Профилиңиз*\n\n"
            f"🆔 ID: `{user_id}`\n"
            f"{vip_text}\n"
            f"👥 Чакырган досторуңуз: {refs}\n\n"
            f"Дос чакыруу шилтемеңиз:\n"
            f"`t.me/PROFIX_bot?start=REF{user_id}`",
            parse_mode="Markdown")

def is_vip_check(user_id):
