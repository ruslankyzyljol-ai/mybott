...
def register_bonus(bot):
    ...
    def bonus_menu(msg):
        ...
        for key, bm in BOOKMAKERS.items():
            kb.add(types.InlineKeyboardButton(
                f"{bm['emoji']} {bm['name']} — {bm['bonus']}", callback_data=f"bonus_{key}"))
        bot.send_message(msg.chat.id, "🎁 *Бонустар:*\n\nТандаңыз 👇",
    ...
    def show_bonus(call):
        ...
        kb = types.InlineKeyboardMarkup()
        kb.add(types.InlineKeyboardButton(f"🚀 {bm['name']} ге кир", url=link))
        kb.add(types.InlineKeyboardButton("⬅️ Артка", callback_data="back_bonus"))
        bot.edit_message_text(
            f"{bm['emoji']} *{bm['name']} Бонус*\n\n"
            f"1️⃣ Ссылкадан кириңиз\n"
            f"2️⃣ Жаңы акк ачыңыз\n"
            f"3️⃣ Промокод жазыңыз: `{promo}`\n"
            f"4️⃣ Депозит коюңуз\n"
            f"✅ *{bm['bonus']}* активдешет!",
    ...
    def back_bonus(call):
        ...
        for key, bm in BOOKMAKERS.items():
            kb.add(types.InlineKeyboardButton(
                f"{bm['emoji']} {bm['name']} — {bm['bonus']}", callback_data=f"bonus_{key}"))
        bot.edit_message_text("🎁 *Бонустар:*\n\nТандаңыз 👇",
            call.message.chat.id, call.message.message_id,
            parse_mode="Markdown", reply_markup=kb)
